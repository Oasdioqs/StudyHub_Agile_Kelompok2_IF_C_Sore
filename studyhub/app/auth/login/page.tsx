...
const nativeGoogleSignIn = async (): Promise<{ email: string; name: string; image: string; googleId: string } | null> => {
  if (!isCapacitor()) return null
  try {
    const pkgName = '@codetrix-studio/capacitor-google-auth'
    const mod = await import(/* webpackIgnore: true */ pkgName).catch(() => null)
    if (!mod?.GoogleAuth) return null
    ...
    <div className="login-wrapper">
      ...
      <div className="card login-card p-4 p-md-5">
        ...
        <button
          ...
          onClick={async () => {
            ...
            if (isCapacitor()) {
              ...
              setError('')
              try {
                const profile = await nativeGoogleSignIn()
                if (!profile) throw new Error('Google sign-in dibatalkan.')
                ...
                // Set session cookie manually for Capacitor
                document.cookie = `next-auth.session-token=${data.token}; path=/; max-age=${data.expiresIn}; SameSite=None; Secure`
                document.cookie = `__Secure-next-auth.session-token=${data.token}; path=/; max-age=${data.expiresIn}; SameSite=None; Secure`
                inputs.successTrigger?.fire?.()
                setSuccess('Login berhasil!')
                setTimeout(() => {
        ...
        >
          ...
          {googleLoading ? 'Signing in...' : 'Masuk dengan Google'}
        </button>
